...
(function () {
    ...
    if (baseUrlOverride) {
      ...
      if (!baseUrlOverride || baseUrlOverride.expiresAt < Date.now()) {
        ...
        logger.call(
          console,
          "To undo run: localStorage.removeItem('togetherjs.baseUrlOverride')"
        );
      }
    }
      ...
      if (!configOverride || configOverride.expiresAt < Date.now()) {
        ...
        for (var attr in configOverride) {
          ...
          if (!shownAny) {
            ...
            console.warn(
              "To undo run: localStorage.removeItem('togetherjs.configOverride')"
            );
          }
    ...
    async function addScript(url) {
      if (url == "/togetherjs/togetherjsPackage.js") {
        import("./bust.js");
        return;
      }
    ...
    var TogetherJS = (window.TogetherJS = function TogetherJS(event) {
      ...
      for (attr in window) {
        if (attr.indexOf("TogetherJSConfig_on_") === 0) {
          ...
          console.warn(
            "TowTruckConfig_* is deprecated, please rename",
            attr,
            "to TogetherJSConfig_on_" + attrName
          );
          globalOns[attrName] = window[attr];
    ...
    function conditionalActivate() {
      ...
      if (!callToStart && window.TowTruckConfig_callToStart) {
        ...
        console.warn(
          "Please rename TowTruckConfig_callToStart to TogetherJSConfig_callToStart"
        );
      }
